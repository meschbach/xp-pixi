...
- [x] 2.1 Implement axial hex math module (`simulation/hex.ts`): neighbor offsets, hex distance, cube rounding — pure,
  no pixels; unit tests for each function
- [x] 2.2 Implement cell-graph map model: cells, six-way adjacency from dimensions, buildable/blocked flags, spawn/goal
  cells; unit tests for interior/edge neighbor counts on a small fixture graph
- [x] 2.3 Implement distance-field computation (BFS from goal over unblocked cells) and recompute-on-blocked-change;
  unit tests incl. blocked rerouting
- [x] 2.4 Implement fixed-timestep world loop (30 Hz accumulator): `tick()` advances enemies modeled as
...
- [x] 2.5 Implement economy + lives state: money, kill rewards, placement costs, lives, loss condition; unit tests for
  each transition

## 3. Waves and combat systems

- [x] 3.1 Define balance data modules in `src/data/` (human-friendly units: seconds for cooldowns/durations, hops for
...
- [x] 3.2 Implement wave spawner driven by data definitions (seven waves in data): timed spawns per wave, wave-clear
...
- [x] 3.3 Implement coverage computation (BFS ≤ R hops through unblocked cells per tower) and targeting:
...
- [x] 3.4 Implement damage/cooldown/kill pipeline wired to economy rewards (damage lands on the attack tick; projectiles
...
- [x] 3.5 Implement placement validation in sim: buildable + unoccupied by tower OR level marker (spawn/goal) +
...
- [x] 4.1 Hex layout renderer: pointy-top tiles from map model, role-based colors (spawn/goal/buildable/blocked/tower),
  redraw on world changes; canvas sized once at load to fit the slice map (fixed size, no resize handling this slice)
- [x] 4.2 Entity visuals: two type-distinct enemy circles (size/color by type) with hp hint, towers (triangles),
...
- [x] 4.3 Click picking: pixel→axial conversion + cube rounding; selection highlight of hovered/clicked cell
- [x] 4.4 Placement UX: hover preview showing would-be coverage and validity before commit; click unoccupied buildable
  tile to place if affordable; rejected placements give visible feedback (no charge)
- [x] 4.5 HUD: money, lives, current wave; game-over and victory overlays with restart button resetting the world to its
  initial state; "Start wave" control for first wave
- [x] 4.6 Wire sim↔render loop: render reads World each animation frame; input dispatches intents into simulation

## 5. Dev experience (HMR/tuning)

- [x] 5.1 Verify Vite HMR on rendering modules updates the running session
- [x] 5.2 Implement tuning hot-apply with explicit `import.meta.hot.accept` boundaries: self-accepting data modules push
...
- [x] 5.3 Add `import.meta.hot.dispose` carry-over for the `World` instance where safe, riding on the same accept
  boundaries; confirm fallback full reload otherwise

## 6. Deployment pipeline

- [x] 6.1 Add GitHub Actions workflow: on PR → typecheck+lint+test+build; on push main → gates + build with `CDN_BASE`
  derived from the repository name (`github.event.repository.name`, so renames/forks don't silently break) +
  upload-pages-artifact + deploy-pages
- [x] 6.2 Enable GitHub Pages with "GitHub Actions" source (repo settings, one-time manual step) and verify first deploy
  serves at the project Pages URL
- [x] 6.3 Validate local production parity: build with non-root `CDN_BASE`, run `vite preview`, confirm game loads at
  subpath
